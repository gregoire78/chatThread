...
function Panel({ channel, chatThreads, scrollBarRefs, chatBans, infosStream, infosChannel, rooms }) {
  ...
  return (
    <>
      {isPopupOpen && <Popup url={window.location + "chat/" + channel} chatThreadChannel={chatThreads.get(channelTag).slice(-50)} closePopup={() => setIsPopupOpen(false)} title={infosChannel && infosChannel.displayName} />}
